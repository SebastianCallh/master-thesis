\documentclass[msc,lith,english]{liuthesis}

\include{settings}

% Packages
\usepackage{rotating}
\usepackage{color}
\usepackage{float}
\usepackage{amsmath}

\DeclareMathOperator*{\argmax}{arg\,max}
\DeclareMathOperator*{\argmin}{arg\,min}

% Notation
\newcommand{\prob}{p}
\newcommand{\model}{\mathcal{M}}
\newcommand{\synchmodel}{\tilde{\mathcal{M}}}
\newcommand{\hyperparam}{\theta}
\newcommand{\obsnoise}{\sigma_n}
\newcommant{\var}{\sigma^2}
\newcommand{\obs}{x}
\newcommand{\stochobs}{X}
\newcommand{\len}{\text{len}}
\newcommand{\dur}{\text{dur}}
\newcommand{\traj}{T}
\newcommand{\newobs}{\bar{x}}
\newcommand{\synchedobs}{X}
\newcommand{\timespace}{t}
\newcommand{\synchspace}{\tau}
\newcommand{\synchslice}{\tau^*}
\newcommand{\statespace}{S}`
\newcommand{\posspace}{S'}
\newcommand{\modelf}{f}
\newcommand{\combinedf}{f^{'}}
\newcommand{\synchf}{g}
\newcommand{\predf}{h}
\newcommand{\arrtime}{t}


\department{Institutionen för datavetenskap}
\departmentenglish{Department of Computer and Information Science}
\departmentshort{IDA}
\supervisor{Mattias Tiger}
\examiner{Fredrik Heintz}
\titleenglish{Trajectory-based Arrival Time Prediction using Gaussian Processes}
\subtitleenglish{}
\titleswedish{Trajektoriebaserad ankomsttidsprediktion med Gaussiska Processer}
\thesissubject{Datateknik}
\publicationyear{2019}
\currentyearthesisnumber{001}
\dateofpublication{2019-05-08}
\author{Sebastian Callh}

\begin{document}
\chapterstyle{VZ43}
\include{content/intro}
\include{content/background}
\include{content/theory}
\include{content/data}
\include{content/method}
\include{content/results}
\include{content/discussion}
\include{content/conclusion}
\printbibliography
\end{document}
