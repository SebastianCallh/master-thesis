...
since different trajectories can have different \textit{lengths} and
\textit{durations}. The length of a trajectory is the amount of
observations in it, and the duration is the time from the first
observation to the last, chronologically. An illustration of the
difficulties brought by this can be seen in Figure~\ref{fig:trajectory-projection-problems}.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{figures/trajectory-projection-problems}
  \caption{An illustration of trajectories $\traj_{i}$ and $\traj_{j}$
...
\end{figure}
Having different lengths places trajectories in different vector spaces,
in which they have no natural comparison using similarity metrics based on Euclidan
